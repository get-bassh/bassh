...
- `Username already taken` → not relevant after signup; means the bassh
...
# → {"success": True, "projects": [{"shortName": "...", "url": "..."}, ...]}
...
For destructive calls (`delete_project`, form `--clear`), confirm with the
user first.

## Tips

- Default to password protection unless the user says otherwise. Bassh URLs
  are public — the password gate is the privacy.
- For multi-page artifacts (HTML + CSS + JS in separate strings), pass them
  as separate files via `deploy_files()` instead of `deploy_html()`. See the
  helper docstring.
- Never echo the user's API key back in the final answer to the user — quote
  the URL and password only.
- The deploy URL is shaped `https://<username>-<project>.pages.dev`. Tell the
  user this is permanent; subsequent deploys to the same project name overwrite.
